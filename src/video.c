...
#define FRAME_SIZE16 (PSP_LINE_SIZE * PSP_SCREEN_HEIGHT * 2)
#define TEXTURE_SIZE (256 * 256 * 2)

#define VOLICON_OFFSET (GBA_SCREEN_HEIGHT + 32)

#define SLICE (64)
#define VERTEX_NUM (2 * 4) // (2 * ceil(GBA_SCREEN_WIDTH / SLICE))
...
static u32 ALIGN_PSPDATA display_list[512] = { 0 };
static u32 ALIGN_PSPDATA display_list_0[256] = { 0 };

static u16 *screen_texture = (u16 *)(0x4000000 + (FRAME_SIZE16 * 2));
...
static float dest_scr_mag;
static u16 dest_scr_x, dest_scr_y, dest_scr_w, dest_scr_h;
...
static void *vram_addr(void *frame, u32 x, u32 y); // un-cached

void load_volume_icon(void);
void draw_volume(int volume);


static void render_scanline_text_base_normal(u32 layer, u32 start, u32 end,
 void *scanline);
static void render_scanline_text_transparent_normal(u32 layer, u32 start,
 u32 end, void *scanline);
static void render_scanline_text_base_color16(u32 layer, u32 start, u32 end,
 void *scanline);
static void render_scanline_text_transparent_color16(u32 layer, u32 start,
 u32 end, void *scanline);
static void render_scanline_text_base_color32(u32 layer, u32 start, u32 end,
 void *scanline);
static void render_scanline_text_transparent_color32(u32 layer, u32 start,
 u32 end, void *scanline);
static void render_scanline_text_base_alpha(u32 layer, u32 start, u32 end,
 void *scanline);
static void render_scanline_text_transparent_alpha(u32 layer, u32 start,
 u32 end, void *scanline);
static void render_scanline_affine_base_normal(u32 layer, u32 start, u32 end,
 void *scanline);
static void render_scanline_affine_transparent_normal(u32 layer, u32 start,
 u32 end, void *scanline);
static void render_scanline_affine_base_color16(u32 layer, u32 start, u32 end,
 void *scanline);
static void render_scanline_affine_transparent_color16(u32 layer, u32 start,
 u32 end, void *scanline);
static void render_scanline_affine_base_color32(u32 layer, u32 start, u32 end,
 void *scanline);
static void render_scanline_affine_transparent_color32(u32 layer, u32 start,
 u32 end, void *scanline);
static void render_scanline_affine_base_alpha(u32 layer, u32 start, u32 end,
 void *scanline);
static void render_scanline_affine_transparent_alpha(u32 layer, u32 start,
 u32 end, void *scanline);
...
static void render_scanline_obj_partial_alpha_1D(u32 priority, u32 start,
 u32 end, render_scanline_dest_partial_alpha *scanline);
...
static void fill_line_color32(u16 color, render_scanline_dest_color32 *dest_ptr,
 u32 start, u32 end);
static void expand_blend(u32 *screen_src_ptr, u16 *screen_dest_ptr, u32 start,
 u32 end);
static void expand_darken(u16 *screen_src_ptr, u16 *screen_dest_ptr, u32 start,
 u32 end);
static void expand_brighten(u16 *screen_src_ptr, u16 *screen_dest_ptr, u32 start,
 u32 end);
...
static void render_scanline_conditional_tile(u32 start, u32 end,
...
const u32 ALIGN_DATA map_widths[] = { 256, 512, 256, 512 };
// const u32 ALIGN_DATA map_heights[] = { 256, 256, 512, 512 };

const u32 ALIGN_DATA obj_width_table[] =
{ 8, 16, 32, 64, 16, 32, 32, 64, 8, 8, 16, 32 };

const u32 ALIGN_DATA obj_height_table[] =
{ 8, 16, 32, 64, 8, 8, 16, 32, 16, 32, 32, 64 };

static u8  ALIGN_DATA obj_priority_list[5][160][128];
static u32 ALIGN_DATA obj_priority_count[5][160];
static u8  ALIGN_DATA obj_alpha_count[160];

static u32 ALIGN_DATA layer_order[16];
u32 layer_count;

const u32 ALIGN_DATA active_layers[6] = { 0x1F, 0x17, 0x1C, 0x14, 0x14, 0x14 };
...
#define partial_tile_right_noflip_8bpp(combine_op, alpha_op)                  \
  ...
  {                                                                           \
    current_pixels =                                                          \
     *((u32 *)(tile_ptr + 4)) >> ((partial_tile_offset - 4) * 8);             \
...
#define multiple_tile_obj(combine_op, color_depth, alpha_op, flip_op)         \
  for(i = 0; i < tile_run; i++)                                               \
  {                                                                           \
    tile_ptr = obj_tile_base + (obj_tile_offset & 0x7FFF);                    \
...
/*  u32 map_height = map_heights[map_size]; */                                    \
  ...
  {                                                                           \
    map_base +=                                                               \
     ((map_width / 8) * 32) + (((vertical_offset - 256) / 8) * 32);           \
  }                                                                           \
  else                                                                        \
  {                                                                           \
    map_base += (((vertical_offset % 256) / 8) * 32);                         \
...
#define obj_scale_offset_1D(color_depth)                                      \
  tile_ptr = obj_tile_base + ((obj_tile_number_1D_##color_depth               \
   + ((vertical_offset / 8) * (max_x / 8) * tile_size_##color_depth)          \
   + ((vertical_offset % 8) * tile_width_##color_depth)) & 0x7FFF)            \

// Get the current row offset into an obj in 2D map space

#define obj_scale_offset_2D(color_depth)                                      \
  tile_ptr = obj_tile_base + ((obj_tile_number_2D_##color_depth               \
   + ((vertical_offset / 8) * 1024)                                           \
   + ((vertical_offset % 8) * tile_width_##color_depth)) & 0x7FFF)            \
...
#define expand_loop(expand_type, effect_condition, pixel_source)              \
  ...
  {                                                                           \
    ...
    {                                                                         \
      *screen_dest_ptr = palette_ram[pixel_source & 0x1FF];                   \
    }                                                                         \
                                                                              \
    screen_src_ptr++;                                                         \
    screen_dest_ptr++;                                                        \
...
#define expand_loop_partial_alpha(alpha_expand, expand_type)                  \
  ...
  {                                                                           \
    ...
    {                                                                         \
      *screen_dest_ptr = palette_ram[pixel_pair & 0x1FF];                     \
    }                                                                         \
                                                                              \
    screen_src_ptr++;                                                         \
    screen_dest_ptr++;                                                        \
...
//  u32 bldcnt = io_registers[REG_BLDCNT];
  ...
  u32 color = palette_ram[0];
  color = (color << 16) || color;
  fill_line_color32(color, scanline32, 0, 120);
...
void update_scanline(void)
{
  ...
  u16 *screen_offset = screen_texture + (vcount * GBA_LINE_SIZE);
...
static void generate_display_list(void)
{
  ...
  vertices =
   (struct Vertex *)sceGuGetMemory(VERTEX_NUM * sizeof(struct Vertex));
  memset(vertices, 0, VERTEX_NUM * sizeof(struct Vertex));
  ...
  for(i = 0; (i + SLICE) < GBA_SCREEN_WIDTH; i += SLICE)
  {
    vertices_tmp[0].u = i;
    vertices_tmp[0].v = 0;
    vertices_tmp[0].x = dest_scr_x + (i * dest_scr_mag);
    vertices_tmp[0].y = dest_scr_y;

    vertices_tmp[1].u = i + SLICE;
    vertices_tmp[1].v = GBA_SCREEN_HEIGHT;
    vertices_tmp[1].x = dest_scr_x + ((i + SLICE) * dest_scr_mag);
    vertices_tmp[1].y = dest_scr_y + dest_scr_h;
...
//if(i < GBA_SCREEN_WIDTH)
  {
    ...
    vertices_tmp[1].v = GBA_SCREEN_HEIGHT;
    vertices_tmp[1].x = dest_scr_x + dest_scr_w;
    vertices_tmp[1].y = dest_scr_y + dest_scr_h;
  }
...
static void bitbilt_sw(void)
{
  ...
  u16 dd1a, dd2a, dd3a, dd4a; // BBBBBxxxxxRRRRR
  u16 dd1b, dd2b, dd3b, dd4b; // xxxxxGGGGGxxxxx

  #define MASK_A 0x7C1F
  #define MASK_B 0x03e0
  ...
  vptr0 = vram_addr(draw_frame, 60, 16);
  d0 = screen_texture;

  for(y = 0; y < GBA_SCREEN_HEIGHT; y += 2)
  {
    vptr = vptr0;
    d = d0 + (y * GBA_LINE_SIZE);

    for(x = 0; x < GBA_SCREEN_WIDTH; x += 2, d += 2)
    {
      vptr[0] = dd1a = dd1b = d[0];
      vptr[2] = dd2a = dd2b = d[1];
      dd1a &= MASK_A; dd1b &= MASK_B;
      dd2a &= MASK_A; dd2b &= MASK_B;

      vptr[0 + PSP_LINE_SIZE * 2] = dd3a = dd3b = d[0 + GBA_LINE_SIZE];
      vptr[2 + PSP_LINE_SIZE * 2] = dd4a = dd4b = d[1 + GBA_LINE_SIZE];
      dd3a &= MASK_A; dd3b &= MASK_B;
      dd4a &= MASK_A; dd4b &= MASK_B;

      *++vptr = (((dd1a + dd2a) >> 1) & MASK_A) |
                (((dd1b + dd2b) >> 1) & MASK_B);
      vptr += (PSP_LINE_SIZE - 1);

      *vptr++ = (((dd1a + dd3a) >> 1) & MASK_A) |
                (((dd1b + dd3b) >> 1) & MASK_B);
      *vptr++ = (((dd1a + dd2a + dd3a + dd4a) >> 2) & MASK_A) |
                (((dd1b + dd2b + dd3b + dd4b) >> 2) & MASK_B);
      *vptr   = (((dd2a + dd4a) >> 1) & MASK_A) |
                (((dd2b + dd4b) >> 1) & MASK_B);
      vptr += (PSP_LINE_SIZE - 1);

      *vptr   = (((dd3a + dd4a) >> 1) & MASK_A) |
                (((dd3b + dd4b) >> 1) & MASK_B);
...
static void set_gba_resolution(void)
{
  switch(option_screen_scale)
  {
    case UNSCALED:
      dest_scr_x = 120;
      dest_scr_y =  56;
      dest_scr_w = GBA_SCREEN_WIDTH;
      dest_scr_h = GBA_SCREEN_HEIGHT;
      dest_scr_mag = 1.0;
    ...
    case SCALED_USER:
      dest_scr_mag = option_screen_mag / 100.0;
      dest_scr_x = GBA_SCREEN_WIDTH - ((GBA_SCREEN_WIDTH / 2) * dest_scr_mag);
      dest_scr_y =
       (GBA_SCREEN_HEIGHT - ((GBA_SCREEN_HEIGHT / 2) * dest_scr_mag)) - 24;
      dest_scr_w = GBA_SCREEN_WIDTH  * dest_scr_mag;
      dest_scr_h = GBA_SCREEN_HEIGHT * dest_scr_mag;
...
{
  u32 clear_color = ((color & 0x001F) << 3) |
                    ((color & 0x03e0) << 6) |
                    ((color & 0x7C00) << 9) | (0xFF << 24);
...
{
  return (void *)(((u32)draw_frame | 0x44000000) + ((x + (y << 9)) << 1));
}

u16 *copy_screen(void)
{
  u16 *copy = malloc(GBA_SCREEN_WIDTH * GBA_SCREEN_HEIGHT * 2);

  u16 *src_ptr = screen_texture;
  u16 *dist_ptr = copy;
  ...
  {
    for(x = 0; x < GBA_SCREEN_WIDTH; x++, src_ptr++, dist_ptr++)
    {
      *dist_ptr = *src_ptr;
    }
    src_ptr += (GBA_LINE_SIZE - GBA_SCREEN_WIDTH);
  }
...
{
  u16 *src_ptr = src;
  u16 *dest_ptr = vram_addr(draw_frame, dest_x, dest_y);

  u32 line_skip = PSP_LINE_SIZE - w;
  u32 x, y;
...
{
  u32 i;
  u16 *dest_ptr = vram_addr(draw_frame, 0, 0);

  for(i = x1; i <= x2; i++)
  {
    dest_ptr[i + y1 * PSP_LINE_SIZE] = color;
    dest_ptr[i + y2 * PSP_LINE_SIZE] = color;
  }

  for(i = y1; i <= y2; i++)
  {
    dest_ptr[x1 + i * PSP_LINE_SIZE] = color;
    dest_ptr[x2 + i * PSP_LINE_SIZE] = color;
  }
}
...
void draw_box_fill(u32 x1, u32 y1, u32 x2, u32 y2, u16 color)
{
  ...
  for(i = y1; i <= y2; i++)
  {
    for(j=x1; j <= x2; j++)
    {
      dest_ptr[j + i * PSP_LINE_SIZE] = color;
    }
  }
}
...
{
  fbm_printVRAM(vram_addr(draw_frame, 0, 0), PSP_LINE_SIZE, x, y,
...
void load_volume_icon(void)
{
  if (firmware_version >= 0x03050210)
  {
    u32 x, y, alpha;
    u16 *dst, *tex_volicon;
