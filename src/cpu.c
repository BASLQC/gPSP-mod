...
// #define RAM_TRANSLATION_CACHE_SIZE (1024 * 384) // 256K + 32K + 96K
#define RAM_TRANSLATION_CACHE_SIZE (1024 * (256 + 32))
...
static u8 ALIGN_DATA rom_translation_cache[ROM_TRANSLATION_CACHE_SIZE] = { 0 };
u8 *rom_translation_ptr = rom_translation_cache;

static u8 ALIGN_DATA ram_translation_cache[RAM_TRANSLATION_CACHE_SIZE] = { 0 };
...
static u8 ALIGN_DATA bios_translation_cache[BIOS_TRANSLATION_CACHE_SIZE] = { 0 };
...
static const u8 ALIGN_DATA bit_count[256] =
{
...
static const u32 ALIGN_DATA psr_masks[16] =
{
...
static const u8 ALIGN_DATA cpu_modes[32] =
{
...
BLOCK_DATA_TYPE ALIGN_DATA block_data[MAX_BLOCK_SIZE];
BLOCK_EXIT_TYPE ALIGN_DATA block_exits[MAX_EXITS];
...
#define block_lookup_translate_arm(mem_type, smc_enable)                      \
  translation_result                                                          \
   = translate_block_arm(pc, mem_type##_translation_region, smc_enable)       \

#define block_lookup_translate_thumb(mem_type, smc_enable)                    \
  translation_result                                                          \
   = translate_block_thumb(pc, mem_type##_translation_region, smc_enable)     \
...
{                                                                             \
  ...
  {                                                                           \
    ...
    {                                                                         \
      u32 hash_target                                                         \
       = ((pc * 2654435761U) >> 16) & (ROM_BRANCH_HASH_SIZE - 1);             \
    ...
    default:                                                                  \
      ...
      {                                                                       \
        ...
        print_string(buffer, 0xFFFF, 0x0000, 5, 5);                           \
        flip_screen(1);                                                       \
...
#define arm_opcode_swi                                                        \
  (((opcode & 0xF000000) == 0xF000000) && (((opcode >> 16) & 0xFF) < 0xF9))   \
...
#define arm_base_cycles()                                                     \
  cycle_count += code_waitstate_cycles_s[1][pc >> 24]                         \
...
#define thumb_exit_point                                                      \
  (((opcode >= 0xD000) && (opcode < 0xDF00)) ||     /* conditional branch */  \
   (((opcode & 0xFF00) == 0xDF00) && (!swi_hle_handle[opcode & 0xFF]) &&      \
    ((opcode & 0xFF) < 0xF9)) ||      /* SWI */                               \
...
#define thumb_branch_target()                                                 \
  ...
  {                                                                           \
    branch_target = block_end_pc +                                            \
     ((s32)(last_opcode << 21) >> 9) + ((opcode & 0x07FF) << 1);              \
  }                                                                           \
  else                                                                        \
  {                                                                           \
...
#define thumb_base_cycles()                                                   \
  cycle_count += code_waitstate_cycles_s[0][pc >> 24]                         \
...
{                                                                             \
  ...
  {                                                                           \
    flag_status = block_data[block_data_position].flag_data;                  \
    block_data[block_data_position].flag_data = (flag_status & needed_mask);  \
...
s32 translate_block_##type(u32 pc, TRANSLATION_REGION_TYPE translation_region,\
 u32 smc_enable)                                                              \
{                                                                             \
  u32 opcode = 0;                                                             \
  u32 last_opcode;                                                            \
  ...
  s32 i;                                                                      \
  u32 flag_status;                                                            \
  BLOCK_EXIT_TYPE ALIGN_DATA external_block_exits[MAX_EXITS];                 \
                                                                              \
...
void flush_translation_cache_ram(void)
{
  ...
  if(ewram_code_min != 0xFFFFFFFF)
  {
    ...
    {
      memset(ewram + (ewram_code_min_page << 16) + ewram_code_min_offset, 0,
    ...
    else
    {
      ...
      {
        memset(ewram + (i << 16), 0, 0x8000);
      }

      memset(ewram + (ewram_code_max_page << 16), 0, ewram_code_max_offset + 1);
    }
  }
...
void init_cpu(void)
{
  ...
  reg[REG_CPSR] = 0x0000005F; // system mode & desable FIQ
  reg[CPU_HALT_STATE] = CPU_ACTIVE;
