...
u32 execute_spsr_restore_body(u32 address)
{
  ...
  if(((io_registers[REG_IE] & io_registers[REG_IF]) & 0x3FFF) &&
...
#define cycle_arm_multiply_add_yes()                                          \
  cycle_count += cycle_multiply(rs) + 1                                       \
...
#define cycle_arm_multiply_long_add_yes()                                     \
  cycle_count += cycle_multiply(rs) + 2                                       \
...
#define cycle_arm_access_memory_load_u32()                                    \
  cycle_count += DATA_WAITSTATE32N(reg_a0) + 1                                \

#define cycle_arm_access_memory_load_u16()                                    \
  cycle_count += DATA_WAITSTATE16N(reg_a0) + 1                                \
...
#define cycle_arm_access_memory_store_u32()                                   \
  cycle_count += DATA_WAITSTATE32N(reg_a0)                                    \

#define cycle_arm_access_memory_store_u16()                                   \
  cycle_count += DATA_WAITSTATE16N(reg_a0)                                    \
...
/*  check_store_reg_pc_no_flags(rd) */                                            \
  if(rd == REG_PC)                                                            \
  {                                                                           \
    cycle_count += DATA_WAITSTATE32S(addr) + DATA_WAITSTATE32N(addr);         \
    generate_indirect_branch_arm();                                           \
  }                                                                           \
}                                                                             \
...
#define arm_block_memory_adjust_pc_load()                                     \
  ...
  if(reg_list & 0x8000)                                                       \
  {                                                                           \
    cycle_count += DATA_WAITSTATE32S(reg_a0) + DATA_WAITSTATE32N(reg_a0);     \
...
#define arm_block_memory_offset_down_a()                                      \
  mips_emit_addiu(reg_a2, base_reg, (-((word_bit_count(reg_list) - 1) * 4)))  \
...
{                                                                             \
  thumb_decode_##type();                                                      \
  cycle_count += cycle_multiply(_rs);                                         \
...
#define thumb_access_memory_generate_address_reg_imm_sp(offset, reg_rb,       \
 reg_ro)                                                                      \
...
#define cycle_internal_thumb_block_memory_store()                             \

#define cycle_internal_thumb_block_memory_load()                              \
  cycle_count++                                                               \
...
{                                                                             \
  ...
  {                                                                           \
    ...
    cycle_internal_thumb_block_memory_##access_type();                        \
...
{                                                                             \
  ...
  {                                                                           \
    ...
    /* Sizuku Advance (PD) to work */                                         \
    break;                                                                    \
  }                                                                           \
  cycle_count += 2;                                                           \
  generate_swi_hle_handler(comment);                                          \
...
#define arm_und()                                                             \
  cycle_count += 3;                                                           \
  generate_load_pc(reg_a0, (pc + 4));                                         \
  generate_function_call_swap_delay(execute_und)                              \
*/
...
u8 ALIGN_DATA swi_hle_handle[256] =
{
  0x0,    // SWI 0:  SoftReset
...
#define DATA_WAITSTATE16N(_address)                                           \
  data_waitstate_cycles_n[0][_address >> 24]                                  \

#define DATA_WAITSTATE32N(_address)                                           \
  data_waitstate_cycles_n[1][_address >> 24]                                  \

#define DATA_WAITSTATE16S(_address)                                           \
  data_waitstate_cycles_s[0][_address >> 24]                                  \

#define DATA_WAITSTATE32S(_address)                                           \
  data_waitstate_cycles_s[1][_address >> 24]                                  \

#define CODE_WAITSTATE16S(_address)                                           \
  code_waitstate_cycles_s[0][_address >> 24]                                  \

#define CODE_WAITSTATE32S(_address)                                           \
  code_waitstate_cycles_s[1][_address >> 24]                                  \

#define BLOCK_MEMORY_WAITSTATE(_address)                                      \
{                                                                             \
  u8 region = _address >> 24;                                                 \
  if( offset == 0 ||                                                          \
      (region >= 0x08 && region <= 0x0D && (reg_a0 % 0x20000) == 0) )         \
  {                                                                           \
    cycle_count += data_waitstate_cycles_n[1][region];                        \
  }                                                                           \
  else                                                                        \
  {                                                                           \
    cycle_count += data_waitstate_cycles_s[1][region];                        \
  }                                                                           \
}                                                                             \
